...
- `@bwip-js/browser`: ไลบรารีสำหรับสร้างบาร์โค้ดกว่า 100+ รูปแบบ (เช่น Code128, EAN, QR ฯลฯ) ทำงานบนเบราว์เซอร์โดยตรง เหมาะสำหรับการสร้างบาร์โค้ดที่ซับซ้อนและปรับแต่งได้มากกว่า `react-barcode` และ `react-qr-code`
...
- `xlsx`: ไลบรารีจาก SheetJS สำหรับจัดการ Excel

  🚨 **คำเตือน: การติดตั้ง `xlsx` แบบปกติ (`npm install xlsx` หรือ `yarn add xlsx`) มีช่องโหว่ความปลอดภัยระดับสูง**
   * **เหตุผลที่ไม่ควรใช้ `xlsx` จาก npm registry ตรงๆ**
     * เวอร์ชันบน npm อาจมี **ช่องโหว่ความปลอดภัย (High severity vulnerabilities)**
     * SheetJS แนะนำให้ติดตั้งแพ็กเกจตรงจาก **CDN ของทีมพัฒนา** เพื่อความปลอดภัยและอัปเดตล่าสุด
   * **วิธีติดตั้ง (แนะนำ)**
    ...
    ```
    * ⚠️ **หมายเหตุ**: สำหรับ Yarn เวอร์ชันใหม่ อาจเจอ error ว่า

    ```
    Usage Error: It seems you are trying to add a package using a https:... url
    ```
    
    วิธีแก้คือระบุชื่อแพ็กเกจก่อน URL เช่น:
